...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            adopt: function(id) {
                table.set();
                $.modal.confirm("确定审核通过该条" + table.options.modalName + "信息吗？", function() {
                    $.operate.get("/system/report/adopt/" + id);
                });
            },
            no: function(id) {
                table.set();
                $.modal.confirm("确定拒绝该条" + table.options.modalName + "信息吗？", function() {
                    $.operate.get("/system/report/no/" + id);
                });
            },
            report: function(id) {
                table.set();
            ...
            details: function (title, url, width, height) {
                table.set();
                $.modal.openOptions({
                    "title": title,
                    "type": 2,
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.ZengFJ_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
